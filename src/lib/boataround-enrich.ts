/**
 * Boataround detail-page parser + inline enricher.
 *
 * The same parsing logic lives in /api/cron/backfill-boataround-prices,
 * but exposed here as a regular library function so it can be called
 * directly from any server-side route (no HTTP cross-call, no
 * CRON_SECRET handshake) when prices/specs are missing on a row.
 */
...
export function parseBoataroundHtml(html: string): Enriched {
  const out: Enriched = {
    price_per_day: null,
    price_per_week: null,
    country: null,
  ...
  // 3) Spec blob — encoded boat-information object
  ...
  // 4) Equipment / Entertainment / Cockpit arrays
...
/**
 * Fetch + parse + update DB row in one shot. Returns true if any field changed.
 * Silent on error — designed for fire-and-forget background enrichment.
 */
export async function enrichBoataroundBoat(boatId: string, detailUrl: string): Promise<boolean> {
  const db = getDb();
  if (!db) return false;
  const html = await fetchHtml(detailUrl);
  if (!html) return false;
  const p = parseBoataroundHtml(html);
  ...
  const { error } = await db.from("charter_boats").update(patch).eq("id", boatId);
  return !error;
}

/**
 * Fire-and-forget batch enrichment for a list of boat IDs.
 * Each call runs in the background; this function returns immediately.
 * Use to backfill the catalog as users browse.
 */
export function enrichBoatsBackground(
  boats: Array<{ id: string; detail_url: string | null }>,
  limit = 8
): void {
  const candidates = boats.filter((b) => b.id && b.detail_url).slice(0, limit);
  for (const b of candidates) {
    // Intentionally not awaited — fires off, never throws back to caller.
    enrichBoataroundBoat(b.id, b.detail_url as string).catch(() => {
